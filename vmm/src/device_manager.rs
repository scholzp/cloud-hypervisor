...
impl DeviceManager {
    ...
    pub fn create_devices(
        ...
        #[cfg(feature = "fw_cfg")]
        {
            ...
            if !self.boot_order.is_empty() && vm_config.payload.is_some() {
                if vm_config.payload.as_mut().unwrap().fw_cfg_config.is_none() {
                    vm_config.payload.as_mut().unwrap().fw_cfg_config =
                        Some(FwCfgConfig::default());
                }
            }
        }
