...
// ignores it and allocates the initrd at its own address.CHV)
...
// fallback. SeaBIOS prioritizes fw_cfg path etc/table-loader, which we export.  CHV exports ACPI
...
// destination consumed by QEMU’s legacy Linux option ROM. We do not expose a option ROM.
...
mod unit_tests {
    ...
    fn test_x86_cfg_kernel_data() {
        ...
        fw_cfg.write(0, SELECTOR_OFFSET, &[FW_CFG_SETUP_SIZE as u8, 0]);
        ...
        fw_cfg.write(0, SELECTOR_OFFSET, &[FW_CFG_SETUP_DATA as u8, 0]);
        ...
        read_buffer.fill(READ_BUFFER_INIT_VALUE);
        fw_cfg.write(0, SELECTOR_OFFSET, &[FW_CFG_KERNEL_DATA as u8, 0]);
